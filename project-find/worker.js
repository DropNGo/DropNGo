...
const RESULT_SCHEMA = {
  ...
  properties: {
    ...
    findings: { type: "array", items: { type: "object", additionalProperties: false, properties: { title: { type: "string" }, sourceUrl: { type: "string" }, sourceName: { type: "string" }, priceEur: { type: ["number","null"] }, whyMatch: { type: "array", items: { type: "string" } }, caution: { type: "array", items: { type: "string" } } }, required: ["title","sourceUrl","sourceName","priceEur","whyMatch","caution"] } }
  },
...
async function research(request, env) {
  ...
  const system = `You are Project Find, an evidence-first vehicle research engine. Understand the user's natural language, separate hard constraints from preferences, assess market reality, and research current public information. Never invent listings, prices, specifications, or evidence. Every sourceUrl must be a real URL returned by web search. Distinguish facts from inference. For vehicles, think like an experienced automotive researcher and inspection advisor. Never claim a photo proves a hidden accident or mechanical defect. Detail level: ${detailLevel}. User location: ${location || "unknown"}. Return only the requested JSON schema.`;
  const payload = { model: "gpt-5.6", store: false, tools: [{ type: "web_search" }], input: [{ role: "system", content: [{ type: "input_text", text: system }] }, { role: "user", content: [{ type: "input_text", text: query }] }], text: { format: { type: "json_schema", name: "find_research_result", strict: true, schema: RESULT_SCHEMA } } };
...
async function serveApp(request, env) {
  ...
  if (url.pathname !== "/" && url.pathname !== "/index.html") return env.ASSETS.fetch(new Request(url, request));
  const assetUrl = new URL(request.url); assetUrl.pathname = "/index.html";
  const response = await env.ASSETS.fetch(new Request(assetUrl, request));
  if (!response.ok) return response;
  const html = await response.text();
  const injected = html.replace("</body>", '<script src="/js/live-research.js"></script><script src="/js/growth.js"></script></body>');
  return new Response(injected, { status: response.status, headers: new Headers(response.headers) });
}

export default { async fetch(request, env) { const url = new URL(request.url); if (url.pathname === "/api/research") return research(request, env); if (request.method === "OPTIONS") return new Response(null, { status: 204, headers: { "access-control-allow-origin": "*" } }); return serveApp(request, env); } };